...
function blur(values, r) {
  ...
  blur5(values, temp, 0, length3, 1);
  blur5(temp, values, 0, length3, 1);
  blur5(values, temp, 0, length3, 1);
  return values;
}
...
function blurh(blur5, T, S, w, h) {
  for (let y4 = 0, n = w * h; y4 < n; ) {
    blur5(T, S, y4, y4 += w, 1);
  }
}
function blurv(blur5, T, S, w, h) {
...
function blurfImage(radius) {
  ...
  return (T, S, start2, stop, step) => {
    ...
    blur5(T, S, start2 + 0, stop + 0, step);
    blur5(T, S, start2 + 1, stop + 1, step);
    blur5(T, S, start2 + 2, stop + 2, step);
    blur5(T, S, start2 + 3, stop + 3, step);
  };
}
...
function bluri(radius) {
  ...
  return (T, S, start2, stop, step) => {
    ...
    for (let i = start2, j = stop; i <= j; i += step) {
      sum6 += S[Math.min(stop, i + s2)];
      T[i] = sum6 / w;
      sum6 -= S[Math.max(start2, i - s2)];
    }
  };
}
...
function reducer(reduce4) {
  return (values) => reduce4(...values);
}
function cross(...values) {
  const reduce4 = typeof values[values.length - 1] === "function" && reducer(values.pop());
  ...
  while (true) {
    product.push(index5.map((j2, i2) => values[i2][j2]));
    let i = j;
    while (++index5[i] === lengths[i]) {
      if (i === 0) return reduce4 ? product.map(reduce4) : product;
      index5[i--] = 0;
    }
  }
}
...
function cumsum(values, valueof) {
  var sum6 = 0, index5 = 0;
  return Float64Array.from(values, valueof === void 0 ? (v2) => sum6 += +v2 || 0 : (v2) => sum6 += +valueof(v2, index5++, values) || 0);
}
...
function variance(values, valueof) {
  ...
  if (count5 > 1) return sum6 / (count5 - 1);
}
...
function extent(values, valueof) {
  ...
  if (valueof === void 0) {
    for (const value of values) {
      if (value != null) {
        if (min6 === void 0) {
          if (value >= value) min6 = max7 = value;
        } else {
          if (min6 > value) min6 = value;
          if (max7 < value) max7 = value;
        }
      }
    }
  } else {
...
function fcumsum(values, valueof) {
  ...
  return Float64Array.from(
    values,
    valueof === void 0 ? (v2) => adder.add(+v2 || 0) : (v2) => adder.add(+valueof(v2, ++index5, values) || 0)
  );
}
...
function flatten(groups4, keys) {
  for (let i = 1, n = keys.length; i < n; ++i) {
    groups4 = groups4.flatMap((g) => g.pop().map(([key, value]) => [...g, key, value]));
  }
  return groups4;
}
...
function flatRollup(values, reduce4, ...keys) {
  return flatten(rollups(values, reduce4, ...keys), keys);
}
function rollup(values, reduce4, ...keys) {
  return nest(values, identity, reduce4, keys);
}
function rollups(values, reduce4, ...keys) {
  return nest(values, Array.from, reduce4, keys);
}
...
function nest(values, map9, reduce4, keys) {
  return function regroup(values2, i) {
    if (i >= keys.length) return reduce4(values2);
    const groups4 = new InternMap();
    const keyof2 = keys[i++];
    let index5 = -1;
    for (const value of values2) {
      const key = keyof2(value, ++index5, values2);
      const group4 = groups4.get(key);
      if (group4) group4.push(value);
      else groups4.set(key, [value]);
    }
    for (const [key, values3] of groups4) {
      groups4.set(key, regroup(values3, i));
    }
    return map9(groups4);
  }(values, 0);
}
...
function sort(values, ...F) {
  ...
  if (f && f.length !== 2 || F.length > 1) {
    const index5 = Uint32Array.from(values, (d, i) => i);
    if (F.length > 1) {
...
function groupSort(values, reduce4, key) {
  return (reduce4.length !== 2 ? sort(rollup(values, reduce4, key), ([ak, av], [bk, bv]) => ascending(av, bv) || ascending(ak, bk)) : sort(group(values, key), ([ak, av], [bk, bv]) => reduce4(av, bv) || ascending(ak, bk))).map(([key2]) => key2);
}
...
var e5 = Math.sqrt(10);
var e2 = Math.sqrt(2);
function tickSpec(start2, stop, count5) {
  const step = (stop - start2) / Math.max(0, count5), power = Math.floor(Math.log10(step)), error = step / Math.pow(10, power), factor = error >= e10 ? 10 : error >= e5 ? 5 : error >= e2 ? 2 : 1;
  let i1, i2, inc2;
  if (power < 0) {
  ...
  if (i2 < i1 && 0.5 <= count5 && count5 < 2) return tickSpec(start2, stop, count5 * 2);
  return [i1, i2, inc2];
}
function ticks(start2, stop, count5) {
  ...
  const reverse4 = stop < start2, [i1, i2, inc2] = reverse4 ? tickSpec(stop, start2, count5) : tickSpec(start2, stop, count5);
  if (!(i2 >= i1)) return [];
  const n = i2 - i1 + 1, ticks4 = new Array(n);
  if (reverse4) {
    if (inc2 < 0) for (let i = 0; i < n; ++i) ticks4[i] = (i2 - i) / -inc2;
    else for (let i = 0; i < n; ++i) ticks4[i] = (i2 - i) * inc2;
  } else {
    if (inc2 < 0) for (let i = 0; i < n; ++i) ticks4[i] = (i1 + i) / -inc2;
    else for (let i = 0; i < n; ++i) ticks4[i] = (i1 + i) * inc2;
  }
  return ticks4;
}
function tickIncrement(start2, stop, count5) {
  stop = +stop, start2 = +start2, count5 = +count5;
  return tickSpec(start2, stop, count5)[2];
}
function tickStep(start2, stop, count5) {
  stop = +stop, start2 = +start2, count5 = +count5;
  const reverse4 = stop < start2, inc2 = reverse4 ? tickIncrement(stop, start2, count5) : tickIncrement(start2, stop, count5);
  return (reverse4 ? -1 : 1) * (inc2 < 0 ? 1 / -inc2 : inc2);
}

// node_modules/d3/node_modules/d3-array/src/nice.js
function nice(start2, stop, count5) {
  let prestep;
  while (true) {
    const step = tickIncrement(start2, stop, count5);
...
function bin() {
  ...
  function histogram(data) {
    ...
    var bins = new Array(m3 + 1), bin4;
    for (i = 0; i <= m3; ++i) {
      bin4 = bins[i] = [];
      bin4.x0 = i > 0 ? tz[i - 1] : x06;
      bin4.x1 = i < m3 ? tz[i] : x12;
    }
    if (isFinite(step)) {
      if (step > 0) {
        for (i = 0; i < n; ++i) {
...
function max(values, valueof) {
  ...
  if (valueof === void 0) {
    ...
    let index5 = -1;
    for (let value of values) {
      if ((value = valueof(value, ++index5, values)) != null && (max7 < value || max7 === void 0 && value >= value)) {
        max7 = value;
      }
    }
  }
...
function min(values, valueof) {
  ...
  if (valueof === void 0) {
    ...
    let index5 = -1;
    for (let value of values) {
      if ((value = valueof(value, ++index5, values)) != null && (min6 > value || min6 === void 0 && value >= value)) {
        min6 = value;
      }
    }
  }
...
function quickselect(array6, k2, left2 = 0, right2 = Infinity, compare) {
  k2 = Math.floor(k2);
  left2 = Math.floor(Math.max(0, left2));
  right2 = Math.floor(Math.min(array6.length - 1, right2));
  if (!(left2 <= k2 && k2 <= right2)) return array6;
  ...
  while (right2 > left2) {
    if (right2 - left2 > 600) {
      ...
      quickselect(array6, k2, newLeft, newRight, compare);
    }
    const t = array6[k2];
    let i = left2;
    let j = right2;
    swap(array6, left2, k2);
    if (compare(array6[right2], t) > 0) swap(array6, left2, right2);
    while (i < j) {
      swap(array6, i, j), ++i, --j;
      while (compare(array6[i], t) < 0) ++i;
      while (compare(array6[j], t) > 0) --j;
    }
    if (compare(array6[left2], t) === 0) swap(array6, left2, j);
    else ++j, swap(array6, j, right2);
    if (j <= k2) left2 = j + 1;
    if (k2 <= j) right2 = j - 1;
  }
  return array6;
...
function quantileIndex(values, p, valueof = number) {
  if (isNaN(p = +p)) return;
  numbers4 = Float64Array.from(values, (_, i2) => number(valueof(values[i2], i2, values)));
  if (p <= 0) return minIndex(numbers4);
  if (p >= 1) return maxIndex(numbers4);
  var numbers4, index5 = Uint32Array.from(values, (_, i2) => i2), j = numbers4.length - 1, i = Math.floor(j * p);
  quickselect(index5, i, 0, j, (i2, j2) => ascendingDefined(numbers4[i2], numbers4[j2]));
  i = greatest(index5.subarray(0, i + 1), (i2) => numbers4[i2]);
  return i >= 0 ? i : -1;
}

// node_modules/d3/node_modules/d3-array/src/threshold/freedmanDiaconis.js
function thresholdFreedmanDiaconis(values, min6, max7) {
  const c6 = count(values), d = quantile(values, 0.75) - quantile(values, 0.25);
  return c6 && d ? Math.ceil((max7 - min6) / (2 * d * Math.pow(c6, -1 / 3))) : 1;
}

// node_modules/d3/node_modules/d3-array/src/threshold/scott.js
function thresholdScott(values, min6, max7) {
  const c6 = count(values), d = deviation(values);
  return c6 && d ? Math.ceil((max7 - min6) * Math.cbrt(c6) / (3.49 * d)) : 1;
}
...
function mode(values, valueof) {
  ...
  if (valueof === void 0) {
    ...
    let index5 = -1;
    for (let value of values) {
      if ((value = valueof(value, ++index5, values)) != null && value >= value) {
        counts.set(value, (counts.get(value) || 0) + 1);
      }
    }
  }
...
function pairs(values, pairof = pair) {
  ...
  let first = false;
  for (const value of values) {
    if (first) pairs4.push(pairof(previous, value));
    previous = value;
    first = true;
  }
  return pairs4;
...
function least(values, compare = ascending) {
  ...
  if (compare.length === 1) {
    ...
    for (const value of values) {
      if (defined ? compare(value, min6) < 0 : compare(value, value) === 0) {
        min6 = value;
        defined = true;
      }
    }
...
function leastIndex(values, compare = ascending) {
  ...
  for (const value of values) {
    ++index5;
    if (min6 < 0 ? compare(value, value) === 0 : compare(value, minValue) < 0) {
      minValue = value;
      min6 = index5;
    }
  }
...
function scan(values, compare) {
  const index5 = leastIndex(values, compare);
...
function shuffler(random) {
  return function shuffle2(array6, i0 = 0, i1 = array6.length) {
    let m3 = i1 - (i0 = +i0);
    while (m3) {
      const i = random() * m3-- | 0, t = array6[m3 + i0];
...
function transpose(matrix) {
  ...
  for (var i = -1, m3 = min(matrix, length2), transpose4 = new Array(m3); ++i < m3; ) {
    for (var j = -1, n, row = transpose4[i] = new Array(n); ++j < n; ) {
      row[j] = matrix[j][i];
    }
  }
  return transpose4;
...
function map2(values, mapper) {
  ...
  return Array.from(values, (value, index5) => mapper(value, index5, values));
}
...
function reduce(values, reducer2, value) {
  ...
  while ({ done, value: next } = iterator.next(), !done) {
    value = reducer2(value, next, ++index5, values);
  }
  return value;
}
...
function axis(orient, scale2) {
  ...
  function axis2(context) {
    var values = tickValues == null ? scale2.ticks ? scale2.ticks.apply(scale2, tickArguments) : scale2.domain() : tickValues, format3 = tickFormat2 == null ? scale2.tickFormat ? scale2.tickFormat.apply(scale2, tickArguments) : identity_default : tickFormat2, spacing = Math.max(tickSizeInner, 0) + tickPadding, range6 = scale2.range(), range0 = +range6[0] + offset, range1 = +range6[range6.length - 1] + offset, position = (scale2.bandwidth ? center : number2)(scale2.copy(), offset), selection2 = context.selection ? context.selection() : context, path3 = selection2.selectAll(".domain").data([null]), tick = selection2.selectAll(".tick").data(values, scale2).order(), tickExit = tick.exit(), tickEnter = tick.enter().append("g").attr("class", "tick"), line = tick.select("line"), text = tick.select("text");
    ...
    tickExit.remove();
    path3.attr("d", orient === left || orient === right ? tickSizeOuter ? "M" + k2 * tickSizeOuter + "," + range0 + "H" + offset + "V" + range1 + "H" + k2 * tickSizeOuter : "M" + offset + "," + range0 + "V" + range1 : tickSizeOuter ? "M" + range0 + "," + k2 * tickSizeOuter + "V" + offset + "H" + range1 + "V" + k2 * tickSizeOuter : "M" + range0 + "," + offset + "H" + range1);
...
function select_default(select) {
  ...
  for (var groups4 = this._groups, m3 = groups4.length, subgroups = new Array(m3), j = 0; j < m3; ++j) {
...
function selectAll_default(select) {
  ...
  for (var groups4 = this._groups, m3 = groups4.length, subgroups = [], parents = [], j = 0; j < m3; ++j) {
...
function data_default(value, key) {
  ...
  for (var m3 = groups4.length, update = new Array(m3), enter = new Array(m3), exit = new Array(m3), j = 0; j < m3; ++j) {
    var parent = parents[j], group4 = groups4[j], groupLength = group4.length, data = arraylike(value.call(parent, parent && parent.__data__, j, parents)), dataLength = data.length, enterGroup = enter[j] = new Array(dataLength), updateGroup = update[j] = new Array(dataLength), exitGroup = exit[j] = new Array(groupLength);
...
function order_default() {
  for (var groups4 = this._groups, j = -1, m3 = groups4.length; ++j < m3; ) {
...
function sort_default(compare) {
  ...
  for (var groups4 = this._groups, m3 = groups4.length, sortgroups = new Array(m3), j = 0; j < m3; ++j) {
...
function node_default() {
  for (var groups4 = this._groups, j = 0, m3 = groups4.length; j < m3; ++j) {
...
function drag_default() {
  ...
  drag.filter = function(_) {
    return arguments.length ? (filter5 = typeof _ === "function" ? _ : constant_default2(!!_), drag) : filter5;
  };
...
function hslConvert(o) {
  ...
  o = o.rgb();
  var r = o.r / 255, g = o.g / 255, b = o.b / 255, min6 = Math.min(r, g, b), max7 = Math.max(r, g, b), h = NaN, s2 = max7 - min6, l = (max7 + min6) / 2;
  if (s2) {
    if (r === max7) h = (g - b) / s2 + (g < b) * 6;
    else if (g === max7) h = (b - r) / s2 + 2;
    else h = (r - g) / s2 + 4;
    s2 /= l < 0.5 ? max7 + min6 : 2 - max7 - min6;
    h *= 60;
  } else {
    s2 = l > 0 && l < 1 ? 0 : h;
  }
...
function nap() {
  var t05, t15 = taskHead, t22, time2 = Infinity;
  while (t15) {
    if (t15._call) {
      if (time2 > t15._time) time2 = t15._time;
      t05 = t15, t15 = t15._next;
    } else {
      t22 = t15._next, t15._next = null;
      t15 = t05 ? t05._next = t22 : taskHead = t22;
    }
  }
  taskTail = t05;
  sleep(time2);
...
var ENDING = 5;
var ENDED = 6;
function schedule_default(node, name, id2, index5, group4, timing) {
...
function selectAll_default3(select) {
  ...
  for (var groups4 = this._groups, m3 = groups4.length, subgroups = [], parents = [], j = 0; j < m3; ++j) {
    for (var group4 = groups4[j], n = group4.length, node, i = 0; i < n; ++i) {
      if (node = group4[i]) {
        for (var children2 = select.call(node, node.__data__, i, group4), child, inherit2 = get2(node, id2), k2 = 0, l = children2.length; k2 < l; ++k2) {
...
function transition_default() {
  ...
  for (var groups4 = this._groups, m3 = groups4.length, j = 0; j < m3; ++j) {
    for (var group4 = groups4[j], n = group4.length, node, i = 0; i < n; ++i) {
      if (node = group4[i]) {
...
var id = 0;
function Transition(groups4, parents, name, id2) {
  this._groups = groups4;
  this._parents = parents;
  this._name = name;
  this._id = id2;
...
function transition_default2(name) {
  ...
  for (var groups4 = this._groups, m3 = groups4.length, j = 0; j < m3; ++j) {
    for (var group4 = groups4[j], n = group4.length, node, i = 0; i < n; ++i) {
      if (node = group4[i]) {
        schedule_default(node, name, id2, i, group4, timing || inherit(node, id2));
      }
    }
  }
  return new Transition(groups4, this._parents, name, id2);
}
...
function BrushEvent(type2, {
  ...
  Object.defineProperties(this, {
    ...
    mode: { value: mode4, enumerable: true, configurable: true },
    _: { value: dispatch3 }
  });
}
...
function brush(dim) {
  var extent4 = defaultExtent, filter5 = defaultFilter2, touchable = defaultTouchable2, keys = true, listeners = dispatch_default("start", "brush", "end"), handleSize = 6, touchending;
  function brush2(group4) {
    ...
    overlay.enter().append("rect").attr("class", "overlay").attr("pointer-events", "all").attr("cursor", cursors.overlay).merge(overlay).each(function() {
      var extent5 = local2(this).extent;
  ...
  Emitter.prototype = {
    ...
    start: function(event, mode4) {
      if (this.starting) this.starting = false, this.emit("start", event, mode4);
      else this.emit("brush", event);
      return this;
    },
    brush: function(event, mode4) {
      this.emit("brush", event, mode4);
      return this;
    },
    end: function(event, mode4) {
      if (--this.active === 0) delete this.state.emitter, this.emit("end", event, mode4);
      return this;
    },
    emit: function(type2, event, mode4) {
  ...
  function started(event) {
    ...
    var that = this, type2 = event.target.__data__.type, mode4 = (keys && event.metaKey ? type2 = "overlay" : type2) === "selection" ? MODE_DRAG : keys && event.altKey ? MODE_CENTER : MODE_HANDLE, signX = dim === Y ? null : signsX[type2], signY = dim === X ? null : signsY[type2], state = local2(that), extent5 = state.extent, selection2 = state.selection, W = extent5[0][0], w0, w1, N = extent5[0][1], n0, n1, E2 = extent5[1][0], e0, e1, S = extent5[1][1], s0, s1, dx = 0, dy = 0, moving, shifting = signX && signY && keys && event.shiftKey, lockX, lockY, points = Array.from(event.touches || [event], (t) => {
    ...
    function keydowned(event2) {
      switch (event2.keyCode) {
        ...
        case 32: {
          if (mode4 === MODE_HANDLE || mode4 === MODE_CENTER) {
            if (signX < 0) e0 = e1 - dx;
  ...
  brush2.extent = function(_) {
    return arguments.length ? (extent4 = typeof _ === "function" ? _ : constant_default4(number22(_)), brush2) : extent4;
  };
...
function chord(directed, transpose4) {
  var padAngle = 0, sortGroups = null, sortSubgroups = null, sortChords = null;
  function chord2(matrix) {
    var n = matrix.length, groupSums = new Array(n), groupIndex = range2(0, n), chords = new Array(n * n), groups4 = new Array(n), k2 = 0, dx;
    matrix = Float64Array.from({ length: n * n }, transpose4 ? (_, i) => matrix[i % n][i / n | 0] : (_, i) => matrix[i / n | 0][i % n]);
    for (let i = 0; i < n; ++i) {
      let x4 = 0;
    ...
    dx = k2 ? padAngle : tau2 / n;
    {
      ...
      for (const i of groupIndex) {
        ...
        if (directed) {
          ...
          groups4[i] = { index: i, startAngle: x06, endAngle: x4, value: groupSums[i] };
        } else {
...
function bisector2(f) {
  ...
  if (f.length !== 2) {
    compare1 = ascending3;
    compare2 = (d, x4) => ascending3(f(d), x4);
    delta = (d, x4) => f(d) - x4;
  } else {
...
function blurh2(blur5, T, S, w, h) {
  for (let y4 = 0, n = w * h; y4 < n; ) {
    blur5(T, S, y4, y4 += w, 1);
  }
}
function blurv2(blur5, T, S, w, h) {
  for (let x4 = 0, n = w * h; x4 < w; ++x4) {
    blur5(T, S, x4, x4 + n, w);
  }
}
function blurfImage2(radius) {
  const blur5 = blurf2(radius);
  return (T, S, start2, stop, step) => {
    start2 <<= 2, stop <<= 2, step <<= 2;
    blur5(T, S, start2 + 0, stop + 0, step);
    blur5(T, S, start2 + 1, stop + 1, step);
...
function blurf2(radius) {
  const radius0 = Math.floor(radius);
  if (radius0 === radius) return bluri2(radius);
  const t = radius - radius0;
  const w = 2 * radius + 1;
  return (T, S, start2, stop, step) => {
    if (!((stop -= step) >= start2)) return;
...
var e22 = Math.sqrt(2);
function tickSpec2(start2, stop, count5) {
  const step = (stop - start2) / Math.max(0, count5), power = Math.floor(Math.log10(step)), error = step / Math.pow(10, power), factor = error >= e102 ? 10 : error >= e52 ? 5 : error >= e22 ? 2 : 1;
  let i1, i2, inc2;
  if (power < 0) {
    inc2 = Math.pow(10, -power) / factor;
    i1 = Math.round(start2 * inc2);
  ...
  if (i2 < i1 && 0.5 <= count5 && count5 < 2) return tickSpec2(start2, stop, count5 * 2);
  return [i1, i2, inc2];
}
function ticks2(start2, stop, count5) {
  ...
  if (!(count5 > 0)) return [];
  if (start2 === stop) return [start2];
  const reverse4 = stop < start2, [i1, i2, inc2] = reverse4 ? tickSpec2(stop, start2, count5) : tickSpec2(start2, stop, count5);
  if (!(i2 >= i1)) return [];
  const n = i2 - i1 + 1, ticks4 = new Array(n);
  if (reverse4) {
    if (inc2 < 0) for (let i = 0; i < n; ++i) ticks4[i] = (i2 - i) / -inc2;
...
function tickIncrement2(start2, stop, count5) {
  stop = +stop, start2 = +start2, count5 = +count5;
  return tickSpec2(start2, stop, count5)[2];
}

// node_modules/d3-contour/node_modules/d3-array/src/nice.js
function nice2(start2, stop, count5) {
  let prestep;
  while (true) {
    const step = tickIncrement2(start2, stop, count5);
    if (step === prestep || step === 0 || !isFinite(step)) {
      return [start2, stop];
    } else if (step > 0) {
      start2 = Math.floor(start2 / step) * step;
...
function contours_default() {
  ...
  function isorings(values, value, callback) {
    var fragmentByStart = new Array(), fragmentByEnd = new Array(), x4, y4, t05, t15, t22, t32;
    x4 = y4 = -1;
    t15 = above(values[0], value);
    cases[t15 << 1].forEach(stitch);
    while (++x4 < dx - 1) {
      t05 = t15, t15 = above(values[x4 + 1], value);
      cases[t05 | t15 << 1].forEach(stitch);
    }
    cases[t15 << 0].forEach(stitch);
    while (++y4 < dy - 1) {
      x4 = -1;
      t15 = above(values[y4 * dx + dx], value);
      t22 = above(values[y4 * dx], value);
      cases[t15 << 1 | t22 << 2].forEach(stitch);
      while (++x4 < dx - 1) {
        t05 = t15, t15 = above(values[y4 * dx + dx + x4 + 1], value);
        t32 = t22, t22 = above(values[y4 * dx + x4 + 1], value);
        cases[t05 | t15 << 1 | t22 << 2 | t32 << 3].forEach(stitch);
      }
      cases[t15 | t22 << 3].forEach(stitch);
    }
    x4 = -1;
    ...
    function stitch(line) {
      var start2 = [line[0][0] + x4, line[0][1] + y4], end = [line[1][0] + x4, line[1][1] + y4], startIndex = index5(start2), endIndex = index5(end), f, g;
...
function orient2dadapt(ax, ay, bx, by, cx, cy, detsum) {
  ...
  let bvirt, c6, ahi, alo, bhi, blo, _i, _j, _0, s1, s0, t15, t05, u32;
  const acx = ax - cx;
  const bcx = bx - cx;
  const acy = ay - cy;
  const bcy = by - cy;
...
function collinear2(d) {
  ...
  for (let i = 0; i < triangles.length; i += 3) {
    const a4 = 2 * triangles[i], b = 2 * triangles[i + 1], c6 = 2 * triangles[i + 2], cross4 = (coords[c6] - coords[a4]) * (coords[b + 1] - coords[a4 + 1]) - (coords[b] - coords[a4]) * (coords[c6 + 1] - coords[a4 + 1]);
...
function formatDate(date2) {
  var hours2 = date2.getUTCHours(), minutes2 = date2.getUTCMinutes(), seconds4 = date2.getUTCSeconds(), milliseconds4 = date2.getUTCMilliseconds();
  return isNaN(date2) ? "Invalid Date" : formatYear(date2.getUTCFullYear(), 4) + "-" + pad(date2.getUTCMonth() + 1, 2) + "-" + pad(date2.getUTCDate(), 2) + (milliseconds4 ? "T" + pad(hours2, 2) + ":" + pad(minutes2, 2) + ":" + pad(seconds4, 2) + "." + pad(milliseconds4, 3) + "Z" : seconds4 ? "T" + pad(hours2, 2) + ":" + pad(minutes2, 2) + ":" + pad(seconds4, 2) + "Z" : minutes2 || hours2 ? "T" + pad(hours2, 2) + ":" + pad(minutes2, 2) + "Z" : "");
}
...
function autoType(object2) {
  for (var key in object2) {
    ...
    else if (!isNaN(number7 = +value)) value = number7;
...
function link_default(links) {
  var id2 = index3, strength = defaultStrength, strengths, distance = constant_default7(30), distances, nodes, count5, bias, random, iterations2 = 1;
  if (links == null) links = [];
  ...
  function initialize() {
    ...
    for (i = 0, count5 = new Array(n); i < m3; ++i) {
...
function nap2() {
  var t05, t15 = taskHead2, t22, time2 = Infinity;
  while (t15) {
    if (t15._call) {
      if (time2 > t15._time) time2 = t15._time;
      t05 = t15, t15 = t15._next;
      ...
      t22 = t15._next, t15._next = null;
      t15 = t05 ? t05._next = t22 : taskHead2 = t22;
    }
  }
  taskTail2 = t05;
  sleep2(time2);
...
function simulation_default(nodes) {
  var simulation, alpha = 1, alphaMin = 1e-3, alphaDecay = 1 - Math.pow(alphaMin, 1 / 300), alphaTarget = 0, velocityDecay = 0.6, forces = /* @__PURE__ */ new Map(), stepper = timer2(step), event = dispatch_default3("tick", "end"), random = lcg_default();
  if (nodes == null) nodes = [];
  function step() {
    tick();
...
function locale_default(locale5) {
  var group4 = locale5.grouping === void 0 || locale5.thousands === void 0 ? identity_default2 : formatGroup_default(map5.call(locale5.grouping, Number), locale5.thousands + ""), currencyPrefix = locale5.currency === void 0 ? "" : locale5.currency[0] + "", currencySuffix = locale5.currency === void 0 ? "" : locale5.currency[1] + "", decimal = locale5.decimal === void 0 ? "." : locale5.decimal + "", numerals = locale5.numerals === void 0 ? identity_default2 : formatNumerals_default(map5.call(locale5.numerals, String)), percent = locale5.percent === void 0 ? "%" : locale5.percent + "", minus = locale5.minus === void 0 ? "−" : locale5.minus + "", nan = locale5.nan === void 0 ? "NaN" : locale5.nan + "";
  function newFormat(specifier) {
    specifier = formatSpecifier(specifier);
    var fill = specifier.fill, align = specifier.align, sign3 = specifier.sign, symbol = specifier.symbol, zero5 = specifier.zero, width = specifier.width, comma = specifier.comma, precision = specifier.precision, trim = specifier.trim, type2 = specifier.type;
    ...
    function format3(value) {
      ...
      if (comma && !zero5) value = group4(value, Infinity);
      var length3 = valuePrefix.length + value.length + valueSuffix.length, padding = length3 < width ? new Array(width - length3 + 1).join(fill) : "";
      if (comma && zero5) value = group4(padding + value, padding.length ? width - valueSuffix.length : Infinity), padding = "";
      switch (align) {
        case "<":
...
function circleStream(stream, radius, delta, direction, t05, t15) {
  ...
  if (t05 == null) {
    ...
    if (direction > 0 ? t05 < t15 : t05 > t15) t05 += direction * tau5;
  }
...
function link(array6) {
  if (!(n = array6.length)) return;
  var n, i = 0, a4 = array6[0], b;
  while (++i < n) {
    a4.n = b = array6[i];
    b.p = a4;
    a4 = b;
...
function polygonContains_default(polygon, point6) {
  ...
  for (var i = 0, n = polygon.length; i < n; ++i) {
    ...
    for (var j = 0; j < m3; ++j, lambda04 = lambda12, sinPhi03 = sinPhi1, cosPhi03 = cosPhi1, point0 = point1) {
      ...
      sum6.add(atan2(k2 * sign3 * sin2(absDelta), cosPhi03 * cosPhi1 + k2 * cos2(absDelta)));
      ...
      if (antimeridian ^ lambda04 >= lambda ^ lambda12 >= lambda) {
        ...
        var phiArc = (antimeridian ^ delta >= 0 ? -1 : 1) * asin(intersection4[2]);
  ...
  return (angle2 < -epsilon7 || angle2 < epsilon7 && sum6 < -epsilon22) ^ winding & 1;
}
...
function line_default(a4, b, x06, y06, x12, y12) {
  var ax = a4[0], ay = a4[1], bx = b[0], by = b[1], t05 = 0, t15 = 1, dx = bx - ax, dy = by - ay, r;
  r = x06 - ax;
  if (!dx && r > 0) return;
  r /= dx;
  if (dx < 0) {
  ...
  if (t05 > 0) a4[0] = ax + t05 * dx, a4[1] = ay + t05 * dy;
  if (t15 < 1) b[0] = ax + t15 * dx, b[1] = ay + t15 * dy;
  return true;
}
...
function fitExtent(projection2, extent4, object2) {
  return fit(projection2, function(b) {
    var w = extent4[1][0] - extent4[0][0], h = extent4[1][1] - extent4[0][1], k2 = Math.min(w / (b[1][0] - b[0][0]), h / (b[1][1] - b[0][1])), x4 = +extent4[0][0] + (w - k2 * (b[1][0] + b[0][0])) / 2, y4 = +extent4[0][1] + (h - k2 * (b[1][1] + b[0][1])) / 2;
...
function projectionMutator(projectAt) {
  ...
  projection2.fitExtent = function(extent4, object2) {
    return fitExtent(projection2, extent4, object2);
  };
...
function sum_default(value) {
  return this.eachAfter(function(node) {
    var sum6 = +value(node.data) || 0, children2 = node.children, i = children2 && children2.length;
...
function shuffle(array6, random) {
  let m3 = array6.length, t, i;
  while (m3) {
    i = random() * m3-- | 0;
    t = array6[m3];
    array6[m3] = array6[i];
...
var Path3 = class {
  ...
  arcTo(x12, y12, x22, y22, r) {
    ...
    if (this._x1 === null) {
      ...
      let x20 = x22 - x06, y20 = y22 - y06, l21_2 = x21 * x21 + y21 * y21, l20_2 = x20 * x20 + y20 * y20, l21 = Math.sqrt(l21_2), l01 = Math.sqrt(l01_2), l = r * Math.tan((pi5 - Math.acos((l21_2 + l01_2 - l20_2) / (2 * l21 * l01))) / 2), t01 = l / l01, t21 = l / l21;
  ...
  arc(x4, y4, r, a0, a1, ccw) {
    ...
    if (da2 > tauEpsilon2) {
      ...
      this._append`A${r},${r},0,${+(da2 >= pi5)},${cw},${this._x1 = x4 + r * Math.cos(a1)},${this._y1 = y4 + r * Math.sin(a1)}`;
    }
  }
  rect(x4, y4, w, h) {
...
function computeUpperHullIndexes(points) {
  const n = points.length, indexes4 = [0, 1];
  let size = 2, i;
  for (i = 2; i < n; ++i) {
    while (size > 1 && cross_default(points[indexes4[size - 2]], points[indexes4[size - 1]], points[i]) <= 0) --size;
    indexes4[size++] = i;
  }
  return indexes4.slice(0, size);
}
...
var uniform_default = function sourceRandomUniform(source) {
  function randomUniform(min6, max7) {
    min6 = min6 == null ? 0 : +min6;
    max7 = max7 == null ? 1 : +max7;
    if (arguments.length === 1) max7 = min6, min6 = 0;
    else max7 -= min6;
    return function() {
      return source() * max7 + min6;
    };
  }
...
var int_default = function sourceRandomInt(source) {
  function randomInt(min6, max7) {
    if (arguments.length < 2) max7 = min6, min6 = 0;
    min6 = Math.floor(min6);
    max7 = Math.floor(max7) - min6;
    return function() {
      return Math.floor(source() * max7 + min6);
    };
  }
...
var irwinHall_default = function sourceRandomIrwinHall(source) {
  function randomIrwinHall(n) {
    if ((n = +n) <= 0) return () => 0;
    return function() {
      for (var sum6 = 0, i = n; i > 1; --i) sum6 += source();
      return sum6 + i * source();
    };
  }
...
function bisector3(f) {
  ...
  if (f.length !== 2) {
    ...
    compare1 = f === ascending4 || f === descending3 ? f : zero4;
    compare2 = f;
    delta = f;
  }
  function left2(a4, x4, lo = 0, hi = a4.length) {
...
function* numbers3(values, valueof) {
  if (valueof === void 0) {
    for (let value of values) {
      if (value != null && (value = +value) >= value) {
        yield value;
...
function Blur23(blur5) {
  return function(data, rx, ry = rx) {
    ...
    const temp = values.slice();
    if (blurx && blury) {
      blurh3(blurx, temp, values, width, height);
      blurh3(blurx, values, temp, width, height);
      blurh3(blurx, temp, values, width, height);
      blurv3(blury, values, temp, width, height);
      blurv3(blury, temp, values, width, height);
      blurv3(blury, values, temp, width, height);
    } else if (blurx) {
      blurh3(blurx, values, temp, width, height);
      blurh3(blurx, temp, values, width, height);
...
function blurh3(blur5, T, S, w, h) {
  for (let y4 = 0, n = w * h; y4 < n; ) {
    blur5(T, S, y4, y4 += w, 1);
  }
}
function blurv3(blur5, T, S, w, h) {
  for (let x4 = 0, n = w * h; x4 < w; ++x4) {
    blur5(T, S, x4, x4 + n, w);
  }
}
function blurfImage3(radius) {
  const blur5 = blurf3(radius);
  return (T, S, start2, stop, step) => {
    start2 <<= 2, stop <<= 2, step <<= 2;
    blur5(T, S, start2 + 0, stop + 0, step);
    blur5(T, S, start2 + 1, stop + 1, step);
...
function blurf3(radius) {
  const radius0 = Math.floor(radius);
  if (radius0 === radius) return bluri3(radius);
  const t = radius - radius0;
  const w = 2 * radius + 1;
  return (T, S, start2, stop, step) => {
    if (!((stop -= step) >= start2)) return;
...
function compareDefined3(compare = ascending4) {
  if (compare === ascending4) return ascendingDefined3;
  if (typeof compare !== "function") throw new TypeError("compare is not a function");
  return (a4, b) => {
    const x4 = compare(a4, b);
    if (x4 || x4 === 0) return x4;
...
function ascendingDefined3(a4, b) {
  return (a4 == null || !(a4 >= a4)) - (b == null || !(b >= b)) || (a4 < b ? -1 : a4 > b ? 1 : 0);
}
...
var e23 = Math.sqrt(2);
function tickSpec3(start2, stop, count5) {
  const step = (stop - start2) / Math.max(0, count5), power = Math.floor(Math.log10(step)), error = step / Math.pow(10, power), factor = error >= e103 ? 10 : error >= e53 ? 5 : error >= e23 ? 2 : 1;
  let i1, i2, inc2;
  if (power < 0) {
    inc2 = Math.pow(10, -power) / factor;
    i1 = Math.round(start2 * inc2);
  ...
  if (i2 < i1 && 0.5 <= count5 && count5 < 2) return tickSpec3(start2, stop, count5 * 2);
  return [i1, i2, inc2];
}
function ticks3(start2, stop, count5) {
  stop = +stop, start2 = +start2, count5 = +count5;
  if (!(count5 > 0)) return [];
  if (start2 === stop) return [start2];
  const reverse4 = stop < start2, [i1, i2, inc2] = reverse4 ? tickSpec3(stop, start2, count5) : tickSpec3(start2, stop, count5);
  if (!(i2 >= i1)) return [];
  const n = i2 - i1 + 1, ticks4 = new Array(n);
  if (reverse4) {
    if (inc2 < 0) for (let i = 0; i < n; ++i) ticks4[i] = (i2 - i) / -inc2;
...
function tickIncrement3(start2, stop, count5) {
  stop = +stop, start2 = +start2, count5 = +count5;
  return tickSpec3(start2, stop, count5)[2];
}
function tickStep3(start2, stop, count5) {
  stop = +stop, start2 = +start2, count5 = +count5;
  const reverse4 = stop < start2, inc2 = reverse4 ? tickIncrement3(stop, start2, count5) : tickIncrement3(start2, stop, count5);
  return (reverse4 ? -1 : 1) * (inc2 < 0 ? 1 / -inc2 : inc2);
}
...
function quickselect3(array6, k2, left2 = 0, right2 = Infinity, compare) {
  k2 = Math.floor(k2);
  left2 = Math.floor(Math.max(0, left2));
  right2 = Math.floor(Math.min(array6.length - 1, right2));
  if (!(left2 <= k2 && k2 <= right2)) return array6;
  compare = compare === void 0 ? ascendingDefined3 : compareDefined3(compare);
  while (right2 > left2) {
    if (right2 - left2 > 600) {
      const n = right2 - left2 + 1;
      const m3 = k2 - left2 + 1;
      ...
      const newLeft = Math.max(left2, Math.floor(k2 - m3 * s2 / n + sd));
      const newRight = Math.min(right2, Math.floor(k2 + (n - m3) * s2 / n + sd));
      quickselect3(array6, k2, newLeft, newRight, compare);
    }
    const t = array6[k2];
    let i = left2;
    let j = right2;
    swap3(array6, left2, k2);
    if (compare(array6[right2], t) > 0) swap3(array6, left2, right2);
    while (i < j) {
      swap3(array6, i, j), ++i, --j;
      while (compare(array6[i], t) < 0) ++i;
      while (compare(array6[j], t) > 0) --j;
    }
    if (compare(array6[left2], t) === 0) swap3(array6, left2, j);
    else ++j, swap3(array6, j, right2);
    if (j <= k2) left2 = j + 1;
    if (k2 <= j) right2 = j - 1;
  }
  return array6;
}
function swap3(array6, i, j) {
  const t = array6[i];
  array6[i] = array6[j];
  array6[j] = t;
}
...
function quantile3(values, p, valueof) {
  values = Float64Array.from(numbers3(values, valueof));
  if (!(n = values.length) || isNaN(p = +p)) return;
  if (p <= 0 || n < 2) return min4(values);
  if (p >= 1) return max5(values);
  var n, i = (n - 1) * p, i0 = Math.floor(i), value0 = max5(quickselect3(values, i0).subarray(0, i0 + 1)), value1 = min4(values.subarray(i0 + 1));
  return value0 + (value1 - value0) * (i - i0);
}
function quantileSorted3(values, p, valueof = number4) {
  if (!(n = values.length) || isNaN(p = +p)) return;
  if (p <= 0 || n < 2) return +valueof(values[0], 0, values);
  if (p >= 1) return +valueof(values[n - 1], n - 1, values);
  var n, i = (n - 1) * p, i0 = Math.floor(i), value0 = +valueof(values[i0], i0, values), value1 = +valueof(values[i0 + 1], i0 + 1, values);
...
function range5(start2, stop, step) {
  start2 = +start2, stop = +stop, step = (n = arguments.length) < 2 ? (stop = start2, start2 = 0, 1) : n < 3 ? 1 : +step;
  var i = -1, n = Math.max(0, Math.ceil((stop - start2) / step)) | 0, range6 = new Array(n);
  while (++i < n) {
    range6[i] = start2 + i * step;
...
function ordinal() {
  var index5 = new InternMap(), domain = [], range6 = [], unknown = implicit;
  function scale2(d) {
    let i = index5.get(d);
    if (i === void 0) {
...
function band() {
  ...
  function rescale() {
    ...
    var values = range5(n).map(function(i) {
...
function polymap(domain, range6, interpolate) {
  ...
  return function(x4) {
    var i2 = bisect_default3(domain, x4, 1, j) - 1;
    return r[i2](d[i2](x4));
  };
}
...
function transformer2() {
  var domain = unit, range6 = unit, interpolate = value_default, transform2, untransform, unknown, clamp = identity5, piecewise2, output, input;
  function rescale() {
    var n = Math.min(domain.length, range6.length);
    if (clamp !== identity5) clamp = clamper(domain[0], domain[n - 1]);
  ...
  scale2.domain = function(_) {
    return arguments.length ? (domain = Array.from(_, number5), rescale()) : domain.slice();
  };
  scale2.range = function(_) {
...
var formatTypes_default2 = {
  ...
  "p": function(x4, p) {
    return formatRounded_default2(x4 * 100, p);
  },
...
function locale_default2(locale5) {
  var group4 = locale5.grouping === void 0 || locale5.thousands === void 0 ? identity_default5 : formatGroup_default2(map8.call(locale5.grouping, Number), locale5.thousands + ""), currencyPrefix = locale5.currency === void 0 ? "" : locale5.currency[0] + "", currencySuffix = locale5.currency === void 0 ? "" : locale5.currency[1] + "", decimal = locale5.decimal === void 0 ? "." : locale5.decimal + "", numerals = locale5.numerals === void 0 ? identity_default5 : formatNumerals_default2(map8.call(locale5.numerals, String)), percent = locale5.percent === void 0 ? "%" : locale5.percent + "", minus = locale5.minus === void 0 ? "-" : locale5.minus + "", nan = locale5.nan === void 0 ? "NaN" : locale5.nan + "";
  function newFormat(specifier) {
    specifier = formatSpecifier2(specifier);
    var fill = specifier.fill, align = specifier.align, sign3 = specifier.sign, symbol = specifier.symbol, zero5 = specifier.zero, width = specifier.width, comma = specifier.comma, precision = specifier.precision, trim = specifier.trim, type2 = specifier.type;
    if (type2 === "n") comma = true, type2 = "g";
    else if (!formatTypes_default2[type2]) precision === void 0 && (precision = 12), trim = true, type2 = "g";
    if (zero5 || fill === "0" && align === "=") zero5 = true, fill = "0", align = "=";
    var prefix = symbol === "$" ? currencyPrefix : symbol === "#" && /[boxX]/.test(type2) ? "0" + type2.toLowerCase() : "", suffix = symbol === "$" ? currencySuffix : /[%p]/.test(type2) ? percent : "";
...
function tickFormat(start2, stop, count5, specifier) {
  var step = tickStep3(start2, stop, count5), precision;
  specifier = formatSpecifier2(specifier == null ? ",f" : specifier);
  switch (specifier.type) {
    case "s": {
      var value = Math.max(Math.abs(start2), Math.abs(stop));
      if (specifier.precision == null && !isNaN(precision = precisionPrefix_default2(step, value))) specifier.precision = precision;
    ...
    case "p":
    case "r": {
      if (specifier.precision == null && !isNaN(precision = precisionRound_default2(step, Math.max(Math.abs(start2), Math.abs(stop))))) specifier.precision = precision - (specifier.type === "e");
      break;
    }
    case "f":
    case "%": {
      if (specifier.precision == null && !isNaN(precision = precisionFixed_default2(step))) specifier.precision = precision - (specifier.type === "%") * 2;
      break;
    }
  }
...
function linearish(scale2) {
  ...
  scale2.ticks = function(count5) {
    var d = domain();
    return ticks3(d[0], d[d.length - 1], count5 == null ? 10 : count5);
  };
  scale2.tickFormat = function(count5, specifier) {
    var d = domain();
    return tickFormat(d[0], d[d.length - 1], count5 == null ? 10 : count5, specifier);
  };
  scale2.nice = function(count5) {
    if (count5 == null) count5 = 10;
    var d = domain();
    var i0 = 0;
    var i1 = d.length - 1;
    var start2 = d[i0];
    ...
    while (maxIter-- > 0) {
      step = tickIncrement3(start2, stop, count5);
      if (step === prestep) {
        d[i0] = start2;
        d[i1] = stop;
        return domain(d);
...
function identity6(domain) {
  ...
  scale2.domain = scale2.range = function(_) {
    return arguments.length ? (domain = Array.from(_, number5), scale2) : domain.slice();
  };
  scale2.unknown = function(_) {
    return arguments.length ? (unknown = _, scale2) : unknown;
  };
  ...
  domain = arguments.length ? Array.from(domain, number5) : [0, 1];
  return linearish(scale2);
}
...
function loggish(transform2) {
  ...
  scale2.ticks = (count5) => {
    ...
    if (!(base % 1) && j - i < n) {
      ...
      if (z.length * 2 < n) z = ticks3(u4, v2, n);
    } else {
      z = ticks3(i, j, Math.min(j - i, n)).map(pows);
    }
    return r ? z.reverse() : z;
  };
  scale2.tickFormat = (count5, specifier) => {
    if (count5 == null) count5 = 10;
    if (specifier == null) specifier = base === 10 ? "s" : ",";
    if (typeof specifier !== "function") {
      if (!(base % 1) && (specifier = formatSpecifier2(specifier)).precision == null) specifier.trim = true;
      specifier = format2(specifier);
    }
    if (count5 === Infinity) return specifier;
    const k2 = Math.max(1, base * count5 / scale2.ticks().length);
    return (d) => {
...
function powish(transform2) {
  var scale2 = transform2(identity5, identity5), exponent2 = 1;
  function rescale() {
    return exponent2 === 1 ? transform2(identity5, identity5) : exponent2 === 0.5 ? transform2(transformSqrt, transformSquare) : transform2(transformPow(exponent2), transformPow(1 / exponent2));
  }
...
function radial() {
  ...
  scale2.range = function(_) {
    return arguments.length ? (squared.range((range6 = Array.from(_, number5)).map(square)), scale2) : range6.slice();
  };
  scale2.rangeRound = function(_) {
...
function quantile4() {
  ...
  function rescale() {
    ...
    while (++i < n) thresholds[i - 1] = quantileSorted3(domain, i / n);
    return scale2;
  }
  function scale2(x4) {
    return x4 == null || isNaN(x4 = +x4) ? unknown : range6[bisect_default3(thresholds, x4)];
  }
  ...
  scale2.copy = function() {
    return quantile4().domain(domain).range(range6).unknown(unknown);
  };
  return initRange.apply(scale2, arguments);
}
...
function quantize() {
  ...
  function scale2(x4) {
    return x4 != null && x4 <= x4 ? range6[bisect_default3(domain, x4, 0, n)] : unknown;
  }
  function rescale() {
    var i = -1;
    domain = new Array(n);
...
function timeInterval(floori, offseti, count5, field) {
  ...
  if (count5) {
    ...
    interval3.every = (step) => {
      ...
      return !isFinite(step) || !(step > 0) ? null : !(step > 1) ? interval3 : interval3.filter(field ? (d) => field(d) % step === 0 : (d) => interval3.count(0, d) % step === 0);
    };
  }
  return interval3;
}
...
function ticker(year2, month2, week, day2, hour2, minute2) {
  ...
  function ticks4(start2, stop, count5) {
    ...
    const ticks5 = interval3 ? interval3.range(start2, +stop + 1) : [];
    return reverse4 ? ticks5.reverse() : ticks5;
  }
  function tickInterval(start2, stop, count5) {
    const target = Math.abs(stop - start2) / count5;
    const i = bisector3(([, , step2]) => step2).right(tickIntervals, target);
    if (i === tickIntervals.length) return year2.every(tickStep3(start2 / durationYear, stop / durationYear, count5));
    if (i === 0) return millisecond.every(Math.max(tickStep3(start2, stop, count5), 1));
...
function formatLocale(locale5) {
  ...
  function newFormat(specifier, formats2) {
    return function(date2) {
      var string = [], i = -1, j = 0, n = specifier.length, c6, pad4, format3;
      if (!(date2 instanceof Date)) date2 = /* @__PURE__ */ new Date(+date2);
      while (++i < n) {
        if (specifier.charCodeAt(i) === 37) {
          string.push(specifier.slice(j, i));
          if ((pad4 = pads[c6 = specifier.charAt(++i)]) != null) c6 = specifier.charAt(++i);
          else pad4 = c6 === "e" ? " " : "0";
          if (format3 = formats2[c6]) c6 = format3(date2, pad4);
          string.push(c6);
          j = i + 1;
        }
      }
...
function formatLookup(names) {
  var map9 = {}, i = -1, n = names.length;
  while (++i < n) map9[names[i].toLowerCase()] = i;
  return map9;
}
...
function calendar(ticks4, tickInterval, year2, month2, week, day2, hour2, minute2, second4, format3) {
  ...
  function tickFormat2(date2) {
    return (second4(date2) < date2 ? formatMillisecond : minute2(date2) < date2 ? formatSecond : hour2(date2) < date2 ? formatMinute : day2(date2) < date2 ? formatHour : month2(date2) < date2 ? week(date2) < date2 ? formatDay : formatWeek : year2(date2) < date2 ? formatMonth : formatYear4)(date2);
  }
  scale2.invert = function(y4) {
    return new Date(invert(y4));
  };
  scale2.domain = function(_) {
    return arguments.length ? domain(Array.from(_, number6)) : domain().map(date);
  };
  scale2.ticks = function(interval3) {
    var d = domain();
    return ticks4(d[0], d[d.length - 1], interval3 == null ? 10 : interval3);
  };
  scale2.tickFormat = function(count5, specifier) {
  ...
  scale2.nice = function(interval3) {
    ...
    return interval3 ? domain(nice4(d, interval3)) : scale2;
  };
  scale2.copy = function() {
    return copy(scale2, calendar(ticks4, tickInterval, year2, month2, week, day2, hour2, minute2, second4, format3));
  };
  return scale2;
}
function time() {
...
function transformer3() {
  var x06 = 0, x12 = 1, t05, t15, k10, transform2, interpolator = identity5, clamp = false, unknown;
  function scale2(x4) {
    return x4 == null || isNaN(x4 = +x4) ? unknown : interpolator(k10 === 0 ? 0.5 : (x4 = (transform2(x4) - t05) * k10, clamp ? Math.max(0, Math.min(1, x4)) : x4));
  }
  scale2.domain = function(_) {
    return arguments.length ? ([x06, x12] = _, t05 = transform2(x06 = +x06), t15 = transform2(x12 = +x12), k10 = t05 === t15 ? 0 : 1 / (t15 - t05), scale2) : [x06, x12];
  };
  scale2.clamp = function(_) {
...
function sequential() {
  var scale2 = linearish(transformer3()(identity5));
  scale2.copy = function() {
...
function sequentialQuantile() {
  ...
  function scale2(x4) {
    if (x4 != null && !isNaN(x4 = +x4)) return interpolator((bisect_default3(domain, x4, 1) - 1) / (domain.length - 1));
  }
  scale2.domain = function(_) {
    if (!arguments.length) return domain.slice();
    domain = [];
...
function transformer4() {
  var x06 = 0, x12 = 0.5, x22 = 1, s2 = 1, t05, t15, t22, k10, k21, interpolator = identity5, transform2, clamp = false, unknown;
  function scale2(x4) {
    return isNaN(x4 = +x4) ? unknown : (x4 = 0.5 + ((x4 = +transform2(x4)) - t15) * (s2 * x4 < s2 * t15 ? k10 : k21), interpolator(clamp ? Math.max(0, Math.min(1, x4)) : x4));
  }
  scale2.domain = function(_) {
    return arguments.length ? ([x06, x12, x22] = _, t05 = transform2(x06 = +x06), t15 = transform2(x12 = +x12), t22 = transform2(x22 = +x22), k10 = t05 === t15 ? 0 : 0.5 / (t15 - t05), k21 = t15 === t22 ? 0 : 0.5 / (t22 - t15), s2 = t15 < t05 ? -1 : 1, scale2) : [x06, x12, x22];
  };
  scale2.clamp = function(_) {
    return arguments.length ? (clamp = !!_, scale2) : clamp;
  };
  ...
  return function(t) {
    transform2 = t, t05 = t(x06), t15 = t(x12), t22 = t(x22), k10 = t05 === t15 ? 0 : 0.5 / (t15 - t05), k21 = t15 === t22 ? 0 : 0.5 / (t22 - t15), s2 = t15 < t05 ? -1 : 1;
    return scale2;
  };
}
function diverging() {
  var scale2 = linearish(transformer4()(identity5));
  scale2.copy = function() {
...
function arc_default() {
  ...
  function arc() {
    ...
    if (!(r1 > epsilon9)) context.moveTo(0, 0);
    else if (da2 > tau7 - epsilon9) {
      ...
      var a01 = a0, a11 = a1, a00 = a0, a10 = a1, da0 = da2, da1 = da2, ap = padAngle.apply(this, arguments) / 2, rp = ap > epsilon9 && (padRadius ? +padRadius.apply(this, arguments) : sqrt3(r0 * r0 + r1 * r1)), rc = min5(abs4(r1 - r0) / 2, +cornerRadius.apply(this, arguments)), rc0 = rc, rc1 = rc, t05, t15;
      if (rp > epsilon9) {
        var p02 = asin2(rp / r0 * sin3(ap)), p1 = asin2(rp / r1 * sin3(ap));
        if ((da0 -= p02 * 2) > epsilon9) p02 *= cw ? 1 : -1, a00 += p02, a10 -= p02;
        else da0 = 0, a00 = a10 = (a0 + a1) / 2;
        if ((da1 -= p1 * 2) > epsilon9) p1 *= cw ? 1 : -1, a01 += p1, a11 -= p1;
      ...
      if (rc > epsilon9) {
        ...
        if (da2 < pi6) {
          if (oc = intersect(x01, y01, x004, y004, x11, y11, x10, y10)) {
            ...
            rc0 = min5(rc, (r0 - lc) / (kc - 1));
            rc1 = min5(rc, (r1 - lc) / (kc + 1));
          } else {
            rc0 = rc1 = 0;
          }
        }
      }
      if (!(da1 > epsilon9)) context.moveTo(x01, y01);
        ...
        if (rc1 < rc) context.arc(t05.cx, t05.cy, rc1, atan22(t05.y01, t05.x01), atan22(t15.y01, t15.x01), !cw);
        else {
      ...
      if (!(r0 > epsilon9) || !(da0 > epsilon9)) context.lineTo(x10, y10);
        ...
        if (rc0 < rc) context.arc(t05.cx, t05.cy, rc0, atan22(t05.y01, t05.x01), atan22(t15.y01, t15.x01), !cw);
        else {
  ...
  arc.centroid = function() {
    var r = (+innerRadius.apply(this, arguments) + +outerRadius.apply(this, arguments)) / 2, a4 = (+startAngle.apply(this, arguments) + +endAngle.apply(this, arguments)) / 2 - pi6 / 2;
...
function line_default2(x4, y4) {
  var defined = constant_default10(true), context = null, curve = linear_default, output = null, path3 = withPath(line);
...
function area_default5(x06, y06, y12) {
  var x12 = null, defined = constant_default10(true), context = null, curve = linear_default, output = null, path3 = withPath(area);
...
function pie_default() {
  var value = identity_default6, sortValues = descending_default, sort4 = null, startAngle = constant_default10(0), endAngle = constant_default10(tau7), padAngle = constant_default10(0);
  function pie(data) {
    var i, n = (data = array_default3(data)).length, j, k2, sum6 = 0, index5 = new Array(n), arcs = new Array(n), a0 = +startAngle.apply(this, arguments), da2 = Math.min(tau7, Math.max(-tau7, endAngle.apply(this, arguments) - a0)), a1, p = Math.min(Math.abs(da2) / n, padAngle.apply(this, arguments)), pa = p * (da2 < 0 ? -1 : 1), v2;
    for (i = 0; i < n; ++i) {
      if ((v2 = arcs[index5[i] = i] = +value(data[i], i, data)) > 0) {
        sum6 += v2;
      }
    }
    if (sortValues != null) index5.sort(function(i2, j2) {
      return sortValues(arcs[i2], arcs[j2]);
    });
    else if (sort4 != null) index5.sort(function(i2, j2) {
      return sort4(data[i2], data[j2]);
    });
    for (i = 0, k2 = sum6 ? (da2 - n * pa) / sum6 : 0; i < n; ++i, a0 = a1) {
      j = index5[i], v2 = arcs[j], a1 = a0 + (v2 > 0 ? v2 * k2 : 0) + pa, arcs[j] = {
        data: data[j],
        index: i,
        value: v2,
        startAngle: a0,
...
function link2(curve) {
  ...
  function link3() {
    let buffer;
    const argv = slice6.call(arguments);
...
var asterisk_default = {
  draw(context, size) {
    const r = sqrt3(size + min5(size / 28, 0.75)) * 0.59436;
    const t = r / 2;
    const u4 = t * sqrt32;
    context.moveTo(0, r);
    context.lineTo(0, -r);
...
var circle_default3 = {
  draw(context, size) {
    const r = sqrt3(size / pi6);
    context.moveTo(r, 0);
    context.arc(0, 0, r, 0, tau7);
  }
};
...
var plus_default = {
  draw(context, size) {
    const r = sqrt3(size - min5(size / 7, 2)) * 0.87559;
    context.moveTo(-r, 0);
    context.lineTo(r, 0);
    context.moveTo(0, r);
    context.lineTo(0, -r);
...
var kr = sin3(pi6 / 10) / sin3(7 * pi6 / 10);
var kx = sin3(tau7 / 10) * kr;
var ky = -cos3(tau7 / 10) * kr;
var star_default = {
  draw(context, size) {
...
var times_default = {
  draw(context, size) {
    const r = sqrt3(size - min5(size / 6, 1.7)) * 0.6189;
    context.moveTo(-r, -r);
    context.lineTo(r, r);
    context.moveTo(-r, r);
    context.lineTo(r, -r);
...
MonotoneX.prototype = {
  ...
  point: function(x4, y4) {
    ...
    switch (this._point) {
      ...
      case 2:
        this._point = 3;
        point5(this, slope2(this, t15 = slope3(this, x4, y4)), t15);
        break;
      default:
        point5(this, this._t0, t15 = slope3(this, x4, y4));
        break;
    }
    this._x0 = this._x1, this._x1 = x4;
    this._y0 = this._y1, this._y1 = y4;
...
function ascending_default2(series) {
  var sums = series.map(sum5);
  return none_default2(series).sort(function(a4, b) {
    return sums[a4] - sums[b];
  });
}
function sum5(series) {
...
function insideOut_default(series) {
  var n = series.length, i, j, sums = series.map(sum5), order = appearance_default(series), top2 = 0, bottom2 = 0, tops = [], bottoms = [];
  for (i = 0; i < n; ++i) {
    j = order[i];
    if (top2 < bottom2) {
      top2 += sums[j];
...
function timeInterval2(floori, offseti, count5, field) {
  function interval3(date2) {
    return floori(date2 = arguments.length === 0 ? /* @__PURE__ */ new Date() : /* @__PURE__ */ new Date(+date2)), date2;
  }
  interval3.floor = (date2) => {
...
var millisecond3 = timeInterval2(() => {
}, (date2, step) => {
  date2.setTime(+date2 + step);
}, (start2, end) => {
  return end - start2;
});
millisecond3.every = (k2) => {
  k2 = Math.floor(k2);
  if (!isFinite(k2) || !(k2 > 0)) return null;
  if (!(k2 > 1)) return millisecond3;
  return timeInterval2((date2) => {
    date2.setTime(Math.floor(date2 / k2) * k2);
  }, (date2, step) => {
    date2.setTime(+date2 + step * k2);
...
var second3 = timeInterval2((date2) => {
  date2.setTime(date2 - date2.getMilliseconds());
}, (date2, step) => {
...
var utcMinute3 = timeInterval2((date2) => {
  date2.setUTCSeconds(0, 0);
}, (date2, step) => {
  date2.setTime(+date2 + step * durationMinute3);
}, (start2, end) => {
...
var utcHour3 = timeInterval2((date2) => {
  date2.setUTCMinutes(0, 0, 0);
}, (date2, step) => {
  date2.setTime(+date2 + step * durationHour3);
}, (start2, end) => {
...
var utcMonth3 = timeInterval2((date2) => {
  date2.setUTCDate(1);
  date2.setUTCHours(0, 0, 0, 0);
}, (date2, step) => {
  date2.setUTCMonth(date2.getUTCMonth() + step);
...
timeYear2.every = (k2) => {
  return !isFinite(k2 = Math.floor(k2)) || !(k2 > 0) ? null : timeInterval2((date2) => {
    date2.setFullYear(Math.floor(date2.getFullYear() / k2) * k2);
    date2.setMonth(0, 1);
    date2.setHours(0, 0, 0, 0);
  }, (date2, step) => {
...
function ticker2(year2, month2, week, day2, hour2, minute2) {
  ...
  function tickInterval(start2, stop, count5) {
    ...
    if (i === tickIntervals.length) return year2.every(tickStep(start2 / durationYear2, stop / durationYear2, count5));
    if (i === 0) return millisecond3.every(Math.max(tickStep(start2, stop, count5), 1));
    const [t, step] = tickIntervals[target / tickIntervals[i - 1][2] < tickIntervals[i][2] / target ? i - 1 : i];
    return t.every(step);
  }
  return [ticks4, tickInterval];
}
var [utcTicks2, utcTickInterval2] = ticker2(utcYear3, utcMonth3, utcSunday3, unixDay2, utcHour3, utcMinute3);
...
function formatLocale2(locale5) {
  ...
  function newFormat(specifier, formats2) {
    return function(date2) {
      ...
      while (++i < n) {
        if (specifier.charCodeAt(i) === 37) {
          string.push(specifier.slice(j, i));
          if ((pad4 = pads2[c6 = specifier.charAt(++i)]) != null) c6 = specifier.charAt(++i);
          else pad4 = c6 === "e" ? " " : "0";
          if (format3 = formats2[c6]) c6 = format3(date2, pad4);
          string.push(c6);
          j = i + 1;
  ...
  function parseSpecifier(d, specifier, string, j) {
    ...
    while (i < n) {
      ...
      if (c6 === 37) {
        c6 = specifier.charAt(i++);
        parse = parses[c6 in pads2 ? specifier.charAt(i++) : c6];
        if (!parse || (j = parse(d, string, j)) < 0) return -1;
      } else if (c6 != string.charCodeAt(j++)) {
        return -1;
      }
...
function formatDayOfYear2(d, p) {
  return pad3(1 + timeDay2.count(timeYear2(d), d), p, 3);
}
...
function formatWeekNumberSunday2(d, p) {
  return pad3(timeSunday2.count(timeYear2(d) - 1, d), p, 2);
}
function dISO2(d) {
  var day2 = d.getDay();
...
function formatWeekNumberISO2(d, p) {
  d = dISO2(d);
  return pad3(timeThursday2.count(timeYear2(d), d) + (timeYear2(d).getDay() === 4), p, 2);
}
function formatWeekdayNumberSunday2(d) {
  return d.getDay();
}
function formatWeekNumberMonday2(d, p) {
  return pad3(timeMonday2.count(timeYear2(d) - 1, d), p, 2);
}
...
function formatUTCDayOfYear2(d, p) {
  return pad3(1 + utcDay3.count(utcYear3(d), d), p, 3);
}
...
function formatUTCWeekNumberSunday2(d, p) {
  return pad3(utcSunday3.count(utcYear3(d) - 1, d), p, 2);
}
function UTCdISO2(d) {
  var day2 = d.getUTCDay();
...
function formatUTCWeekNumberISO2(d, p) {
  d = UTCdISO2(d);
  return pad3(utcThursday3.count(utcYear3(d), d) + (utcYear3(d).getUTCDay() === 4), p, 2);
}
function formatUTCWeekdayNumberSunday2(d) {
  return d.getUTCDay();
}
function formatUTCWeekNumberMonday2(d, p) {
  return pad3(utcMonday3.count(utcYear3(d) - 1, d), p, 2);
}
...
function defaultConstrain(transform2, extent4, translateExtent) {
  var dx0 = transform2.invertX(extent4[0][0]) - translateExtent[0][0], dx1 = transform2.invertX(extent4[1][0]) - translateExtent[1][0], dy0 = transform2.invertY(extent4[0][1]) - translateExtent[0][1], dy1 = transform2.invertY(extent4[1][1]) - translateExtent[1][1];
...
function zoom_default2() {
  ...
  zoom.scaleTo = function(selection2, k2, p, event) {
    zoom.transform(selection2, function() {
      var e = extent4.apply(this, arguments), t05 = this.__zoom, p02 = p == null ? centroid(e) : typeof p === "function" ? p.apply(this, arguments) : p, p1 = t05.invert(p02), k1 = typeof k2 === "function" ? k2.apply(this, arguments) : k2;
      return constrain(translate(scale2(t05, k1), p02, p1), e, translateExtent);
    }, p, event);
  };
  ...
  zoom.translateTo = function(selection2, x4, y4, p, event) {
    zoom.transform(selection2, function() {
      var e = extent4.apply(this, arguments), t = this.__zoom, p02 = p == null ? centroid(e) : typeof p === "function" ? p.apply(this, arguments) : p;
  ...
  function schedule(transition2, transform2, point6, event) {
    transition2.on("start.zoom", function() {
      ...
      var that = this, args = arguments, g = gesture(that, args).event(event), e = extent4.apply(that, args), p = point6 == null ? centroid(e) : typeof point6 === "function" ? point6.apply(that, args) : point6, w = Math.max(e[1][0] - e[0][0], e[1][1] - e[0][1]), a4 = that.__zoom, b = typeof transform2 === "function" ? transform2.apply(that, args) : transform2, i = interpolate(a4.invert(p).concat(w / a4.k), b.invert(p).concat(w / b.k));
      return function(t) {
        if (t === 1) t = b;
        else {
          var l = i(t), k2 = w / l[2];
  ...
  function dblclicked(event, ...args) {
    if (!filter5.apply(this, arguments)) return;
    var t05 = this.__zoom, p02 = pointer_default(event.changedTouches ? event.changedTouches[0] : event, this), p1 = t05.invert(p02), k1 = t05.k * (event.shiftKey ? 0.5 : 2), t15 = constrain(translate(scale2(t05, k1), p02, p1), extent4.apply(this, args), translateExtent);
    noevent_default3(event);
    if (duration > 0) select_default2(this).transition().duration(duration).call(schedule, t15, p02, event);
    else select_default2(this).call(zoom.transform, t15, p02, event);
  }
  ...
  zoom.filter = function(_) {
    return arguments.length ? (filter5 = typeof _ === "function" ? _ : constant_default11(!!_), zoom) : filter5;
  };
  zoom.touchable = function(_) {
    return arguments.length ? (touchable = typeof _ === "function" ? _ : constant_default11(!!_), zoom) : touchable;
  };
  zoom.extent = function(_) {
    return arguments.length ? (extent4 = typeof _ === "function" ? _ : constant_default11([[+_[0][0], +_[0][1]], [+_[1][0], +_[1][1]]]), zoom) : extent4;
  };
...
export {
  ...
  isoFormat_default2 as isoFormat,
  isoParse_default2 as isoParse,
  ...
  timeFridays2 as timeFridays,
  timeHour2 as timeHour,
  timeHours2 as timeHours,
  timeInterval2 as timeInterval,
  millisecond3 as timeMillisecond,
  milliseconds3 as timeMilliseconds,
  timeMinute2 as timeMinute,
  timeMinutes2 as timeMinutes,
  timeMonday2 as timeMonday,
  timeMondays2 as timeMondays,
  timeMonth2 as timeMonth,
  timeMonths2 as timeMonths,
  timeParse2 as timeParse,
  timeSaturday2 as timeSaturday,
  timeSaturdays2 as timeSaturdays,
  second3 as timeSecond,
  seconds3 as timeSeconds,
  timeSunday2 as timeSunday,
  timeSundays2 as timeSundays,
  timeThursday2 as timeThursday,
  timeThursdays2 as timeThursdays,
  timeTickInterval2 as timeTickInterval,
  timeTicks2 as timeTicks,
  timeTuesday2 as timeTuesday,
  timeTuesdays2 as timeTuesdays,
  timeWednesday2 as timeWednesday,
  timeWednesdays2 as timeWednesdays,
  timeSunday2 as timeWeek,
  timeSundays2 as timeWeeks,
  timeYear2 as timeYear,
  timeYears2 as timeYears,
  ...
  union,
  unixDay2 as unixDay,
  unixDays2 as unixDays,
  utcDay3 as utcDay,
  utcDays3 as utcDays,
  utcFormat2 as utcFormat,
  utcFriday3 as utcFriday,
  utcFridays3 as utcFridays,
  utcHour3 as utcHour,
  utcHours3 as utcHours,
  millisecond3 as utcMillisecond,
  milliseconds3 as utcMilliseconds,
  utcMinute3 as utcMinute,
  utcMinutes3 as utcMinutes,
  utcMonday3 as utcMonday,
  utcMondays3 as utcMondays,
  utcMonth3 as utcMonth,
  utcMonths3 as utcMonths,
  utcParse2 as utcParse,
  utcSaturday3 as utcSaturday,
  utcSaturdays3 as utcSaturdays,
  second3 as utcSecond,
  seconds3 as utcSeconds,
  utcSunday3 as utcSunday,
  utcSundays3 as utcSundays,
  utcThursday3 as utcThursday,
  utcThursdays3 as utcThursdays,
  utcTickInterval2 as utcTickInterval,
  utcTicks2 as utcTicks,
  utcTuesday3 as utcTuesday,
  utcTuesdays3 as utcTuesdays,
  utcWednesday3 as utcWednesday,
  utcWednesdays3 as utcWednesdays,
  utcSunday3 as utcWeek,
  utcSundays3 as utcWeeks,
  utcYear3 as utcYear,
  utcYears3 as utcYears,
  variance,
